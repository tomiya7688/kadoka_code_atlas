...
def test_source_config_lives_under_config_directory():
    assert _default_config_path() == Path(app.__file__).resolve().parent / "config" / "kadoka-code-atlas.json"


def test_frozen_config_is_resolved_beside_exe_under_config(monkeypatch, tmp_path):
    exe = tmp_path / "kadoka-code-atlas.exe"
    monkeypatch.setattr(sys, "frozen", True, raising=False)
    monkeypatch.setattr(sys, "executable", str(exe))
    assert _default_config_path() == tmp_path / "config" / "kadoka-code-atlas.json"
